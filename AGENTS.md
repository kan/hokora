...
さらに、**root module のタグを打つと nested module である `sdk/` はその
module から除外される。** つまり sdk タグを打たずにサーバーだけリリースすると、
`go get github.com/kan/hokora/sdk` は解決先を失う。

手順(順序を守ること):

1. リリース準備コミット(CHANGELOG の版を確定)を push する
2. **`sdk/vX.Y.Z` を先に打って push する**
3. 続けて **`vX.Y.Z`** を同一コミットに打って push する
   (`release.yml` が同番号・同コミットの sdk タグの存在を検査し、
   無ければリリースを止める)

番号の方針:

- **マイナー・パッチは連動させる。** 同一コミットに同じ番号を打つ。
  互換は `/v1` のパスで決まっており版番号は互換を語っていないので、
  情報量ではなく **打ち忘れを検査できること** を優先する。SDK に変更が
  無い回でも番号を揃えて打つ
- **メジャーは連動させない。** Go の module は v2 以降で import パスに
  接尾辞が付くため、**サーバー側の破壊的変更で `sdk/v2` に上げると、SDK に
  何の変更もないのに利用側が import を書き換えさせられる。** sdk の
  メジャーは Go API の破壊でのみ上げ、その時点で番号体系を分離する
