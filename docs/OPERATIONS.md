...
### 8.3 `hokora get`（端末確認用）

```bash
sudo hokora get DATABASE_URL         # 値を 1 行 stdout に出す
```

- **端末での確認専用。** `hokora get KEY > file` のような**ファイル生成に使わない**
  （`.env` 出力＝`export` を実装しない方針と同じ理由）。

### 8.4 `hokora run`（既存アプリの移行用）

```bash
sudo hokora run -- /usr/local/bin/legacy-app --flag
```
...
- **限界（重要）:** 環境変数は **`/proc/<pid>/environ` から読める**。T1-a の
  ...
  （THREAT_MODEL R5）。**Go アプリでは SDK 方式を使うこと。** `hokora run` は
  SDK 化できない既存アプリの移行手段と位置づける。
...
#   hokora get --credentials /path/to/creds SOME_KEY
```
