// 開発ツール(golangci-lint / govulncheck)専用の module。
//
// **成果物にリンクされない依存を、利用側の module graph から切り離すための
// 分離である。** `tool` ディレクティブを root の go.mod に置くと、ツールの
// indirect(200 module 超)が root の go.mod に列挙され、SDK しか import
// していない利用側にも module graph として伝播する。
//
// go / toolchain の宣言は root と揃える(`make toolchain-check` が検査する)。
module github.com/kan/hokora/tools
